...
chelidonine:
  ...
  name: chelidonine
  bits_set: [ 36,48,66,68,77,117,328,418,447,458,461,465,466,470,476,481,482,483,484,488,489,493,499,
    506,513,514,519,678,684,712,714,720,727,785,814,879,895,902,1072,1101,1132,1135,1141,1168,1179,1190,
    1208,1210,1211,1216,1224,1225,1226,1227,1228,1230,1231,1240,1242,1304,1325,1346,1347,1410,1706,3670,
    4357,4489,4816,4823,4967,4999,5004,5014,5088,5090,5111,5118,5148,5158,5196,5334,5364,5695,5701,6228,
    6617,8202,8219,8230,8232,8240,8248,8254,8261,8276,8290,8297,8302,8310,8311,8315,8321,8332,8340,8341,
    8360,8469,8606,8857,8984,9031,9058,9184,9202,9239,9252,9370,9413,9441,9465,9762,9783,9885,10058,10070,
    10183,10286,10343,10666,10776,10817,10833,10908,11080,11186,11197,11253,11309,11390,11404,11653,11795,
    11797,11834,11996,12157,12216,12270,12277,12545,12620,12841,12875,12915,13365,13389,13401,13470,13585,
    13626,13651,13715,13722,13733,13839,13854,13929,13959,14159,14505,14722,17138]
fluorocompound:
  smiles: "N[C@@](F)(C)C(=O)O"
  smiles_flat: "O=C(O)C(F)(N)C"
  name: fluorothing
  bits_set: [ 77,78,117,138,179,415,445,456,468,473,484,495,512,519,814,879,902,1064,2564,4357,4433,
    4736,4810,4822,4863,5158,5196,5364,5694,5703,7302,8399,8433,8441,9017,9098,9771,10024,10243,10250,
    10766,17786
    ]
...
class FingerprintingTest(unittest.TestCase):
    ...
    def test_calc_one_fingerprint(self):
        smiles = self.data["fluorocompound"]["smiles"]
        smiles_processed = self.fingerprinter.process([smiles], 
                                                      calc_fingerprint = True)
        self.assertEqual(len(smiles_processed), 1)
        self.assertEqual(smiles_processed[0]["data_id"], 0)
        self.assertEqual(smiles_processed[0]["smiles_canonical"],
                         self.data["fluorocompound"]["smiles_flat"])
