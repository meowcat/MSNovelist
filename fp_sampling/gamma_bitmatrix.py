...
class SamplerFactory:
    ...
    def get_sampler(self):
        if not self._loaded:
            self.load()
        sampler = GammaBitmatrixRandomBinarySampler(self.bitmatrix_stats)
        return sampler
...
class GammaBitmatrixRandomBinarySampler(Sampler):
    ...
    def sample(self, fp):
        ...
        if not self.reverse_0_stats:
            bitmatrix_gamma_0 = \
                tf.expand_dims(self.bitmatrix_stats[0,:], 0) ** \
                tf.expand_dims(gamma, 1)
        else:
            bitmatrix_gamma_0 = 1 - (
                (1 - tf.expand_dims(self.bitmatrix_stats[0,:], 0)) ** \
                tf.expand_dims(gamma, 1)
            )
        bitmatrix_gamma_1 = \
            tf.expand_dims(self.bitmatrix_stats[1,:], 0) ** \
            tf.expand_dims(gamma, 1)
