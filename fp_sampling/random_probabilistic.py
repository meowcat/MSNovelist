...
class SamplerFactory:
    ...
    def _load(self):
        ...
        submap = (self.base_folder + 
                      "csi_current_release/statistics/train.csv")
        data_submap = pd.read_csv(submap, header=None, sep='\t')
        subindex =[d in self.fp_map.data.index for d in data_submap[0]]
        self.fp_map.subset_map(data_submap[0].loc[subindex])
        # Load the actual fingerprint table
        data_eval_ = (self.base_folder + 
                      "csi_current_release/predictions/crossvalidation_open.csv")
        self.db_eval = db.FpCsv(data_eval_, 
                                fp_map = self.fp_map.positions,
                                fingerprinter = None)
        self.db_eval.randomize(random_seed = 44)
        # Add groups
        data_gnps_groups_ = (self.base_folder + 
                            "gnps_evaluation_data/gnps-evaluation-instances.csv")
        data_gnps_groups = pd.read_csv(data_gnps_groups_, delimiter="\t")
        data_gnps_groups.set_index('GNPS-identifier', inplace=True)
        self.data_gnps_groups = data_gnps_groups
        self.loaded = True
    ...
    def get_sampler(self):
        ...
        db_eval_select = self.db_eval.data_information.reindex(
            pd.Index.difference(self.db_eval.data_information.index, 
                                self.data_gnps_groups.index))

        fp_map = self.fp_map.positions
        
        fp_true = self.db_eval.data_fp_true[db_eval_select.row_id,:][:,fp_map]
        fp_predicted = self.db_eval.data_fp_predicted[db_eval_select.row_id,:][:,fp_map]
        
        sampler = RandomProbabilisticSampler(
            fp_true,
            fp_predicted,
            noise = 0.15)
        return sampler
...
class RandomProbabilisticSampler(Sampler):
